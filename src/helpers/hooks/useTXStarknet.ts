...
export function useTXStarknet() {
  const { address } = useAccount();
  // const { chain } = useNetwork();

  const { contract } = useContract({
    abi: STARKNET_ERC721_ABI,
    address:
      "0x048d1f460c9aa44e2a2cd4d49982b41e8ba10174f966c227b213aed77814808d",
  });

  const calls = useMemo(() => {
    if (!address || !contract) return [];
    return contract.populateTransaction["mintCollectible"]!(
      address
      //     {
      //   low: 1,
      //   high: 0,
  ...
  const { writeAsync, data, isPending, ...result } = useContractWrite({
    calls,
  });
  console.log(result, "REST OF RESULT");
...
const STARKNET_ERC721_ABI = [
  ...
  {
    type: "event",
    name: "test_ozeppellin_cairo::erc721::SeasonPass::Event",
    kind: "enum",
    variants: [
      {
        name: "ERC721Event",
